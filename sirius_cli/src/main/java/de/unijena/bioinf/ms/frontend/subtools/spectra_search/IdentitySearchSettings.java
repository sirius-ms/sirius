...
@AllArgsConstructor
@NoArgsConstructor
public class IdentitySearchSettings extends SpectralLibrarySearchSettings implements Ms2ExperimentAnnotation {
    @Getter
    protected boolean enabled;

    public IdentitySearchSettings(boolean enabled, float minSimilarity, int minNumOfPeaks, int maxNumOfHits, SpectralMatchingType matchingType, Deviation precursorDeviation, Set<SpectrumType> queryTypes, Set<SpectrumType> targetTypes) {
        super(minSimilarity, minNumOfPeaks, maxNumOfHits, matchingType, precursorDeviation, queryTypes, targetTypes);
        this.enabled = enabled;
    }

    @DefaultInstanceProvider
    public static IdentitySearchSettings newInstance(
        ...
        return new IdentitySearchSettings(enabled, minSimilarity, minNumOfPeaks, maxNumOfHits, matchingType, precursorDeviation, queryTypes, targetTypes);
    }
}
