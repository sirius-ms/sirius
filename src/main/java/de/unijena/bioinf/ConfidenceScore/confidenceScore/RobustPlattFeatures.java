 ...
 */
public class RobustPlattFeatures implements FeatureCreator {
    ...
    public double[] computeFeatures(CompoundWithAbstractFP<ProbabilityFingerprint> query, CompoundWithAbstractFP<Fingerprint>[] rankedCandidates) {
        ...
        TDoubleArrayList plattList = new TDoubleArrayList();
        int j = 0;
        for (FPIter fpIter : query.getFingerprint().iterator()) {
            if (statistics[j].getSmallerClassSize()>=100 && statistics[j].getF()>=0.6){
                plattList.add(fpIter.getProbability());
            }
            ++j;
        }
        final double[] platt = plattList.toArray();
        Arrays.sort(platt);
