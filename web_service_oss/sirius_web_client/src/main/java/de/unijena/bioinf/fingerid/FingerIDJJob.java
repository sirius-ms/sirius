...
public class FingerIDJJob<S extends FormulaScore> extends BasicMasterJJob<List<FingerIdResult>> {
    ...
    protected List<FingerIdResult> compute() throws Exception {
        ...
        final Map<JJob<? extends ResultAnnotation>, FingerIdResult> annotationJJobs = new LinkedHashMap<>(filteredResults.size());
        ...
        for (IdentificationResult<S> fingeridInput : filteredResults) {
            ...
            final WebJJob<FingerprintJobInput,?,FingerprintResult,?> predictionJob = NetUtils.tryAndWait(
            ...
            if(bayesnetScoring != null) {
                ...
                final WebJJob<CovtreeJobInput, ?, BayesnetScoring, ?>  covTreeJob = NetUtils.tryAndWait(() ->
        ...
        checkForInterruption();
        annotationJJobs.forEach((k,v) -> v.takeAndAnnotate(k));
        checkForInterruption();
