...
public class FingerblastJJob extends FingerprintDependentJJob<FingerblastResult> {
    ...
    public synchronized void handleFinishedRequiredJob(JJob required) {
        ...
        if(bayesnetScoring == null){
            if(required.result() instanceof BayesnetScoring)
                bayesnetScoring = (BayesnetScoring) required.result();
        }
    }
