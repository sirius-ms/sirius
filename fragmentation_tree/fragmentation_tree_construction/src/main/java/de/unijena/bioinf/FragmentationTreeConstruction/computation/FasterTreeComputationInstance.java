...
public class FasterTreeComputationInstance extends AbstractTreeComputationInstance {
    ...
    protected void recalculateScore(FTree tree) {
        ...
        if (Math.abs(newScore - oldScore) > 0.1) {
            if (pinput.getAnnotation(TreeSizeScorer.TreeSizeBonus.class,null)!=null){
                ...
                this.LOG().warn("Score of " + tree.getRoot().getFormula() + " differs significantly from recalculated score: " + oldScore + " vs " + newScore + " with tree sort key is score " + tree.getTreeWeight() + " and filename is " + String.valueOf(pinput.getExperimentInformation().getSource()));
            }
        }
    }
