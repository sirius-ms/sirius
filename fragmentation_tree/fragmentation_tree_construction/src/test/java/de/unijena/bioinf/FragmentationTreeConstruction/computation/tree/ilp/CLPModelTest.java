...
public class CLPModelTest {
    @Test
    public void testCLPModel_JNI() {
        CLPModel_JNI model = new CLPModel_JNI(2, CLPModel_JNI.ObjectiveSense.MAXIMIZE);
        ...
        model.addSparseRowCached(row1, new int[] { 0, 1 }, -infty, 20.0);
        double[] row2 = new double[] { 10.0, 7.0 };
        model.addSparseRowCached(row2, new int[] { 0, 1 }, -infty, 35.0);
        double[] row3 = new double[] { 3.0, 4.0 };
        model.addSparseRowCached(row3, new int[] { 0, 1 }, 6.0, infty);
        // solve
        model.setColStart(new double[]{1d, 0d});
        assertEquals(CLPModel_JNI.ReturnStatus.OPTIMAL, model.solve());
