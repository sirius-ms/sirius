...
public class StandardScoring2 implements Scoring<Fragment>, SimpleEqualityScoring<Fragment> {

    public final static float DEFAULT_MATCHSCORE = 5;
    public final static float DEFAULT_SCOREFOREACHNONHYDROGEN = 1;
    public final static float DEFAULT_MISSMATCHPENALTY = -1;
    public final static float DEFAULT_PENALTYFOREACHNONHYDROGEN = -0.25f;

    public final static float DEFAULT_LOSSMATCHSCORE = 5;
    public final static float DEFAULT_LOSSSCOREFOREACHNONHYDROGEN = 1;
    public final static float DEFAULT_LOSSMISSMATCHPENALTY = -1;
    public final static float DEFAULT_LOSSPENALTYFOREACHNONHYDROGEN = -0.25f;
    ...
    public float scoreFormulas(MolecularFormula left, MolecularFormula right, boolean isLoss) {
        ...
        if (diffs > 0 || left.numberOfHydrogens() != right.numberOfHydrogens()) {
            if (left.isSubtractable(right) || right.isSubtractable(left)) {
                final float penalty = (isLoss ? lossMissmatchPenalty : missmatchPenalty);
                final float perHetero = (isLoss ? lossPenaltyForEachNonHydrogen : penaltyForEachNonHydrogen);
                return penalty + perHetero * diffs;
            } else {
                final float penalty = (isLoss ? lossMissmatchPenalty : missmatchPenalty);
                final float perHetero = (isLoss ? lossPenaltyForEachNonHydrogen : penaltyForEachNonHydrogen);
                return penalty + perHetero *
                        Math.max(left.atomCount() - left.numberOfHydrogens(), right.atomCount() - right.numberOfHydrogens());
            }
        } else {
