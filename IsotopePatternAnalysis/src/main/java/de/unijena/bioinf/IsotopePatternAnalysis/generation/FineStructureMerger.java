...
public class FineStructureMerger {
    ...
    public double[] mergeMasses(FinestructureGenerator.Iterator iter, double monoMass) {
        ...
        final TDoubleArrayList peaks = new TDoubleArrayList(masses.length);
        for (int k = 0; k < masses.length; ++k) {
            if (masses[k] != null) {
                final double peakMass = newton(masses[k].toArray(), intensities[k].toArray());
                peaks.add(peakMass);
            }
        }
    ...
    public SimpleSpectrum mergePeaks(FinestructureGenerator.Iterator iter) {
        return null;
    }

    public double newton(double[] normalDistributions, double[] intensities) {
        int basePeak = 0;
