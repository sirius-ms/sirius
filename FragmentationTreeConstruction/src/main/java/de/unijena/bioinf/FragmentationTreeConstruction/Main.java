...
public class Main {
    ...
    private static Pipeline getPipeline() {
        ...
        final ArrayList<PeakPairScorer> closureScorers = new ArrayList<PeakPairScorer>();
        if (LOSS_SIZE==0)
        ...
        pipeline.setPeakPairScorer(new PeakPairScoreList(closureScorers.toArray(new PeakPairScorer[0])));
        pipeline.setEdgeScorer(new EdgeScoreList(edgeScorers.toArray(new LossScorer[edgeScorers.size()])));
