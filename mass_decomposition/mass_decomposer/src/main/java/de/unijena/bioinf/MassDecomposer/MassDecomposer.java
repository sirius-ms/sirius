...
public class MassDecomposer<T> {
    ...
    public boolean maybeDecomposable(double startMass, double endMass) {
        ...
        for (long i = range.getMin(); i <= range.getMax(); ++i) {
            final long r = i % a;
            if (i >= ERT[(int)r][weights.size()-1]) return true;
        }
        return false;
    }
    ...
    protected static class DecompIteratorImpl<T> implements DecompIterator<T> {
        ...
        protected final long a; // mass of first character of the alphabet
        protected int i; // current column in the ERT
        protected long m; // current mass
        protected long currentIntegerMass; // current mass to decompose
        ...
        private boolean decomposable(int i, long m, long a) {
            return (m>=0) && ERT[(int)(m % a)][i] <= m;
        }
