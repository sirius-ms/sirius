    ...
    public int getTraceFor(ProcessedSample sample, Ms2SpectrumHeader ms2) {
        ...
        if (parentId<0) {
            ...
            for (int k=parentId; k >= parentId-10 && k >= 0; --k) {
                SimpleSpectrum ms1Spectrum = sample.getStorage().getSpectrumStorage().getMs2Spectrum(k);
                if (Spectrums.mostIntensivePeakWithin(ms1Spectrum, ms2.getPrecursorMz(), new Deviation(10))>sample.getStorage().getStatistics().noiseLevel(k)) {
                    parentId = k;
                    break;
                }
            }
