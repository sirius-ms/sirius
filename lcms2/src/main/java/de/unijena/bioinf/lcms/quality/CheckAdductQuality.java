...
public class CheckAdductQuality implements FeatureQualityChecker{
    ...
    public void addToReport(QualityReport report, MergedLCMSRun run, AlignedFeatures feature, TraceProvider provider) {
        // majors
        QualityReport.Category peakQuality = new QualityReport.Category(DefaultQualityCategory.ADDUCT_QUALITY);
        ...
        if (size == 1 && !unsure) {
            ...
            + allAdducts.getFirst() + " for this feature", DataQuality.GOOD, QualityReport.Weight.MAJOR));
        } else if (size<=2 && unsure) {
            peakQuality.getItems().add(new QualityReport.Item("There is an unambigious adduct assignment of "
                    + allAdducts.getFirst() + " for this feature, but it's confidence is very low.", DataQuality.DECENT, QualityReport.Weight.MAJOR));
        } else if (size>=2) {
