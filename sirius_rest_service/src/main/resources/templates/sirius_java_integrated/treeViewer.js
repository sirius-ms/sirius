...
var TreeViewer = (function () {
      ...
      if (nodeToMove == null) {
        if (clickedNode.__data__.parent != null) {
          ...
          d3.select('#collapse_button')
            ...
            .attr(
              'transform',
              'translate(' + collapse_x + ',' + collapse_y + ')'
            )
  ...
  function highlightNode(node) {
    ...
    for (var style in styles[theme]['node-rect'])
      svg
        .selectAll('.node rect')
        .style(style, styles[theme]['node-rect'][style]);
  ...
  function highlightNodeById(fragmentId) {
    ...
    var targetNodeSelection = svg
      .selectAll('.node') // Select within the initialized svg
      .filter(function (d) {
        ...
        return (
          ...
          d.data &&
          d.data.fragmentData &&
          d.data.fragmentData.id === fragmentId
        );
      });
    ...
    if (!targetNodeSelection.empty()) {
      ...
      console.warn(
        'Node with fragmentId ' + fragmentId + ' not found in the tree.'
      );
  ...
  function linkAngle(x1, x2, y1, y2) {
    var angle =
      x2 == x1 ? 0 : Math.atan((y2 - y1) / (x2 - x1)) * (180 / Math.PI);
  ...
  function getTransformedCoordinate(coord, axis, reverse = false) {
    if (axis == 'x') {
      if (!reverse)
        return (
          (coord / tree_scale - currentZoom.x / tree_scale) / currentZoom.k
        );
      else
        return (
          (coord * currentZoom.k + currentZoom.x / tree_scale) * tree_scale
        );
    } else {
      if (!reverse)
        return (
          (coord / tree_scale - currentZoom.y / tree_scale) / currentZoom.k
        );
      else
        return (
          (coord * currentZoom.k + currentZoom.y / tree_scale) * tree_scale
        );
    }
  }
  ...
  function collapseNode(node) {
    ...
    for (var i = 0; i < data.losses.length; i++) {
      if (data.losses[i].target == id || data.losses[i].target == name)
        node_loss_i = i;
      else if (data.losses[i].source == id || data.losses[i].source == name) {
  ...
  function colorCode(variant, scheme) {
    ...
    if (max == 0)
      ...
      max = 0.0001;
    var colorScale = d3
      .scaleSequential(scheme_fn)
      .domain([neg ? -max : 0, max]);
