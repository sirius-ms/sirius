...
public class SearchableFieldService {
    ...
    public SearchableFieldService(@NotNull IndexFacts facts, @Nullable FieldVocabulary projectVocabulary) {
        this(facts, projectVocabulary, new SearchableFieldDescriber(ApiDocFieldDescriptions.PROVIDER));
    }
    ...
    public List<SearchableField> describe(@NotNull Class<?> modelClass) {
        ...
        if (!fields.isEmpty() && Taggable.class.isAssignableFrom(modelClass)) {
            ...
            tagValueTypes.keySet().stream().sorted().forEach(tagName -> fields.add(SearchableFields
                    .toTagSearchableField(Taggable.makeTagFieldName(tagName), tagName, tagValueTypes.get(tagName), null)));
        }

        if (projectVocabulary != null) {
            fields.forEach(field -> {
                List<String> possibleValues = projectVocabulary.getPossibleValues(field.getName());
