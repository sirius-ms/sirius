...
public class CompoundMatchHighlighter {
    ...
    public CompoundMatchHighlighter(CompoundCandidate compound, ProbabilityFingerprint fp) {
        compound.parseAndPrepare();
        // we have to match all fingerprints against this compound
        final IAtomContainer container = compound.compound.getMolecule();
        ...
        try {
            for (FPIter mp : compound.compound.fingerprint.presentFingerprints()) {
    ...
    public void hightlight(CompoundCandidate compound) {
        final IAtomContainer molecule = compound.compound.getMolecule();
