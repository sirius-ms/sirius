...
public class SpectralMatchingCache {
    ...
    public synchronized List<SpectralMatchBean> getPage(int page) {
        ...
        if (allMatches != null) {
            ...
            List<SpectralLibraryMatch> matches = instanceBean
                    .withIds((pid, fid) -> instanceBean.getClient().features().getSpectralLibraryMatchesPageWithResponseSpec(pid, fid, pg, PAGE_SIZE, List.of("similarity,desc", "sharedPeaks,desc"), MIN_SHARED_PEAKS, MIN_SIMILARITY, inchiKey, List.of()).bodyToMono(PagedModelSpectralLibraryMatch.class).onErrorComplete().block()).getContent();
    ...
    public synchronized List<SpectralMatchBean> getAll() {
        if (allMatches == null) {
            List<SpectralLibraryMatch> matches = instanceBean
                    .withIds((pid, fid) -> instanceBean.getClient().features().getSpectralLibraryMatchesPageWithResponseSpec(pid, fid, 0, Integer.MAX_VALUE, List.of("similarity,desc", "sharedPeaks,desc"), MIN_SHARED_PEAKS, MIN_SIMILARITY, inchiKey, List.of()).bodyToMono(PagedModelSpectralLibraryMatch.class).onErrorComplete().block()).getContent();
