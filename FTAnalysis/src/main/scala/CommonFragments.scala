...
class CommonFragments(root:String) {
  ...
  def fragmentChemicalPrior = {
    val prior = ChemicalCompoundScorer.createDefaultCompoundScorer(true)
    def score(f:MolecularFormula) = if(f.getMass >= 100) prior.score(f) else 0d
    println(fragments.zip(fragments.map(f=>score(MolecularFormula.parse(f)))).map(x=>"\"" + x._1.toString + "\","+ x._2.toString).mkString(","))
  }
