...
|**alignMaxRetentionTimeDeviation** | **Double** | Maximum allowed retention time error in seconds for aligning features. If not specified, this parameter is estimated from the data. |  [optional] |
|**minSNR** | **Double** | Minimum ratio between peak height and noise intensity for detecting features. By default, this value is 3. Features with good MS/MS are always picked independent of their intensity. For picking very low intensity features we recommend a min-snr of 2, but this will increase runtime and storage requirements |  [optional] |
